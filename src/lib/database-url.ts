 ...
 *
 * Development: 127.0.0.1:3306 固定（Unix ソケット優先）
 * Production:  DB_HOST, DB_PORT（1Password）
 */
...
export function getDatabaseConfig() {
  ...
  const host = isProduction ? trim(process.env.DB_HOST) : DEV_DB_HOST;
  const port = isProduction ? trim(process.env.DB_PORT) : DEV_DB_PORT;
  ...
  return { isProduction, host, port, user, password, name };
}
